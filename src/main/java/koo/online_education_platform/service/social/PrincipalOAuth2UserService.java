...
// => OAuth2 로그인 후 후처리 - 회원 가입, 회원 정보에 따른 등록 등을 수행
// => 소셜 로그인 후 스프링 시큐리티 세션에 로그인 정보를 담는 기능을 담당
/**
 * 다음과 같은 순서로 동작
 * 네이버 로그인 버튼 클릭 -> 네이버 로그인창 -> 로그인 완료 -> code 리턴 (OAuth-client 라이브러리) -> code를 기반으로 AccessToken 요청 -> AccessToken을 기반으로 userRequest 정보 (유저 정보) 반환 -> loadUser 함수 호출 -> 네이버 회원 프로필 받아오기
 */
@Service
@Slf4j
